...
import BulletinAdd from './FileAdd.vue'
import BulletinEdit from './FileEdit.vue'
...
export default {
  ...
  methods: {
    download (row) {
      window.open('http://127.0.0.1:9527/imagesWeb/' + row.images.split(',')[0], '_blank')
    },
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/file-info/' + ids).then(() => {
